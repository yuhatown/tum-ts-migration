...
export const tokenPriceInfo = () => {
    ...
    for (let i = 0; i < token.length; i++) {
        if (token[i].id === "ethereum") {
          const tokenId = i + 1;
          const tokenPrice = new TokenPriceRegister(tokenId, token[i].current_price, token[i].last_updated)
          tokenPrice.register(tokenId, token[i].current_price, token[i].last_updated)
